...
**標準CloudFormation構成**:
- `stacks/`: 親スタック定義（環境ごとにデプロイする単位）
- `templates/`: 再利用可能なネストスタックテンプレート
- `parameters/`: 環境別パラメーター（dev/stg/prod）
- `scripts/`: デプロイ・ロールバックスクリプト
